...
Current snapshot: completed Coolify deployment `b4c1jatqaidljtcpc4nly1yp` and its direct health/publication/browser/Lighthouse observations belong to the older `06491dc…` source and do not validate the current repository candidate. Historical exact-SHA Forgejo runs 61–63 failed for `06491dc…`; later remote dispatches 64–69 for `dc6d78a…` were rejected before runner assignment, and no hosted run exists for the current repository candidate. The registry has no current-candidate image; provider completion, release-specific SLO, rollback, and qualified approvals remain open. The full provider contract failed on Tempo HTTP 503, and Tower environment-contract validation still returns HTTP 500 and needs controller diagnosis. This snapshot supersedes earlier deployment, canonical-url, and CI-run statements below where they differ.
