...
This is an owner-run handoff, not an automated approval. Immutable `v1.0.0-rc.1` remains at `031b5447786f9c619288c9044bb5bc65319a30d7`; the selected untagged RC3 candidate is `06491dc57fa3f3a43c365a4ab24e07f21e7b04d6` on `release/v1-closure`. Never paste passwords, API keys, private keys, DSNs, personal data, legal contracts, or secret-manager values into this repository, Markdown, screenshots, or chat.

Current snapshot: the candidate is deployed at `https://paper-and-slate-web.dev.tower` as completed Coolify deployment `b4c1jatqaidljtcpc4nly1yp`; direct health identity, representative publication/feed checks, and mapped browser smoke are available. A one-off mapped Lighthouse sweep scored all six configured routes at 1.0 across the four categories, with slow-load warnings on `/` and `/projects/file-system`; the standard exact-SHA CI Lighthouse run still fails at root Chrome sandbox startup. Visual comparison and rights approval remain pending, and the hosted visual capture is blocked by image delivery. Exact-SHA Forgejo runs 61–63 failed; the registry has no candidate image; provider completion, release-specific SLO, rollback, and qualified approvals remain open. The full provider contract failed on Tempo HTTP 503, and Tower environment-contract validation still returns HTTP 500 and needs controller diagnosis. This snapshot supersedes earlier deployment, canonical-url, and CI-run statements below where they differ.
...
- A private Forgejo repository exists at `https://git.tower/callum/paperandslate-web.git`; the exact candidate branch source is pushed. Protected review, signed-tag verification, and passing Forgejo CI are not evidenced: runs 61–63 failed.
- Tower-managed staging resources for Typesense `search`, Valkey `cache`, and S3 `release-evidence` have been reconciled. Coolify application `ngqtewtqeqhj88v1005a38va` and deployment `b4c1jatqaidljtcpc4nly1yp` run the exact candidate; six monitors have fresh healthy probes but materialize the homepage path. The deployment remains a direct Coolify build, so no hosted OCI digest/SBOM receipt is claimed.
...
5. Return the exact commit SHA, PR/review ID, tag/signature verification result, and CI run IDs. Current exact-candidate Forgejo runs 61–63 for `06491dc…` are failed; do not treat them as passing evidence until the full Forgejo UI/task logs are inspected.
