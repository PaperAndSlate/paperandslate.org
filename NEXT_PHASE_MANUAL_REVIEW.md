...
This is an owner-run handoff, not an automated approval. Immutable `v1.0.0-rc.1` remains at `031b5447786f9c619288c9044bb5bc65319a30d7`; the current local untagged source candidate is `fa6cc7db48fc262a03349f32270a88ba735113a4` on `release/v1-closure`. The private Forgejo branch last observed by this workspace is `dc6d78a3886079b7fef8816731969fd88c00b755`; the local follow-up has not been pushed. Never paste passwords, API keys, private keys, DSNs, personal data, legal contracts, or secret-manager values into this repository, Markdown, screenshots, or chat.

Current snapshot: completed Coolify deployment `b4c1jatqaidljtcpc4nly1yp` and its direct health/publication/browser/Lighthouse observations belong to the older `06491dc…` source and do not validate local `fa6cc7d…`. Historical exact-SHA Forgejo runs 61–63 failed for `06491dc…`; later remote dispatches 64–69 for `dc6d78a…` were rejected before runner assignment, and no hosted run exists for local `fa6cc7d…`. The registry has no local-candidate image; provider completion, release-specific SLO, rollback, and qualified approvals remain open. The full provider contract failed on Tempo HTTP 503, and Tower environment-contract validation still returns HTTP 500 and needs controller diagnosis. This snapshot supersedes earlier deployment, canonical-url, and CI-run statements below where they differ.
...
- A private Forgejo repository exists at `https://git.tower/callum/paperandslate-web.git`; the older `dc6d78a…` source is the last observed remote branch revision, while local `fa6cc7d…` is unpushed. Protected review, signed-tag verification, and passing Forgejo CI are not evidenced: runs 61–63 failed for `06491dc…`, and dispatches 64–69 for `dc6d78a…` were rejected before runner assignment.
- Tower-managed staging resources for Typesense `search`, Valkey `cache`, and S3 `release-evidence` have been reconciled. Coolify application `ngqtewtqeqhj88v1005a38va` and deployment `b4c1jatqaidljtcpc4nly1yp` run the older `06491dc…` candidate; six monitors have fresh healthy probes but materialize the homepage path. The deployment remains a direct Coolify build, so no hosted OCI digest/SBOM receipt is claimed for local `fa6cc7d…`.
- Exact staging publication checks for `/`, `/projects`, `/standards/changes`, the three feeds, robots, sitemap, AI outputs, projects JSON, and search returned valid 200 responses with no localhost leakage for the older deployment. The health payload reports that deployment’s staging identity, release, and SHA. Typesense is empty/static-fallback; S3/Valkey resource probes pass, but app-level provider evidence remains open and no local-candidate staging receipt exists.
