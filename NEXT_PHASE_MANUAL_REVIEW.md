...
This is an owner-run handoff, not an automated approval. Immutable `v1.0.0-rc.1` remains at `031b5447786f9c619288c9044bb5bc65319a30d7`; the current exact repository candidate is `739a6470f043cac60a0f48bb65be31d9ad07dbed` on `release/v1-closure`, with implementation parent `addc0132a7fabba95c19e45679b40d21dbc72df2`. Local and private Forgejo branch revisions agree; no push was made by this task. Never paste passwords, API keys, private keys, DSNs, personal data, legal contracts, or secret-manager values into this repository, Markdown, screenshots, or chat.

Current snapshot: completed Coolify deployment `b4c1jatqaidljtcpc4nly1yp` and its direct health/publication/browser/Lighthouse observations belong to the older `06491dc…` source and do not validate repository candidate `739a647…`. Historical exact-SHA Forgejo runs 61–63 failed for `06491dc…`; later remote dispatches 64–69 for `dc6d78a…` were rejected before runner assignment, and no hosted run exists for `739a647…`. The registry has no current-candidate image; provider completion, release-specific SLO, rollback, and qualified approvals remain open. The full provider contract failed on Tempo HTTP 503, and Tower environment-contract validation still returns HTTP 500 and needs controller diagnosis. This snapshot supersedes earlier deployment, canonical-url, and CI-run statements below where they differ.
...
- A private Forgejo repository exists at `https://git.tower/callum/paperandslate-web.git`; local and remote branch revisions currently agree at `739a647…`, with implementation parent `addc013…`. Protected review, signed-tag verification, and passing Forgejo CI are not evidenced: runs 61–63 failed for `06491dc…`, and dispatches 64–69 for `dc6d78a…` were rejected before runner assignment.
- Tower-managed staging resources for Typesense `search`, Valkey `cache`, and S3 `release-evidence` have been reconciled. Coolify application `ngqtewtqeqhj88v1005a38va` and deployment `b4c1jatqaidljtcpc4nly1yp` run the older `06491dc…` candidate; six monitors have fresh healthy probes but materialize the homepage path. The deployment remains a direct Coolify build, so no hosted OCI digest/SBOM receipt is claimed for repository candidate `739a647…`.
