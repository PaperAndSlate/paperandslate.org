...
1. **Refresh private Forgejo write authority (owner/operator):** the local handoff commits are `7120874` and `2a7d02a`; a direct HTTPS push was rejected as an expired/invalid credential. Refresh the scoped Forgejo/Tower credential through the approved channel, push `release/v1-closure`, and verify that the remote branch contains the documentation commits. Do not move or overwrite `v1.0.0-rc.1`. Codex can recheck the remote SHA and CI triggers after the credential is refreshed.
2. **Resolve CI (owner/operator):** open the private Forgejo links for runs 17–19 (and the RC1 runs 12–16), inspect full task logs in Forgejo, and correct the provider/workflow failure. Rerun only after the cause is known; retain run IDs, task results, artifacts, and the exact source SHA. Codex can normalize the receipt and update the requirement ledger.
3. **Set the staging canonical URL (Tower/Coolify operator):** configure `NEXT_PUBLIC_SITE_URL=https://paper-and-slate-web.dev.tower` through Coolify's supported native environment-variable surface, because the Tower project-variable API explicitly rejects public/browser-prefixed variables. Redeploy the exact selected SHA and verify that canonical tags, RSS/JSON feeds, robots, sitemap, and JSON-LD contain no `http://localhost:3000`. Codex can run the route/SEO/feed checks and capture the redacted receipt.
4. **Activate or intentionally defer providers (owner/provider operator):** repair the Typesense credential/resource because the shared resource currently returns HTTP 401; then bind only secret names, test search success/outage fallback, and record rotation/retention policy. Separately decide whether Kit, GlitchTip, Valkey, and Infisical are in this release; Codex can run authorized round-trip, idempotency/rate-limit, labeled-error, and fallback tests without receiving secret values.
5. **Create hosted release identity (release/operator):** publish the selected exact source to the private immutable OCI repository, generate image and lockfile SBOMs, record provenance/signature/vulnerability results, and verify deployed digest equality. The current Coolify workload builds directly from Forgejo and the registry has no image. If `59c1e2c...` remains the selected source, create a new RC tag after review; keep RC1 immutable.
6. **Run rollback evidence (security/operations owner):** approve two healthy immutable staging deployments and an A→B→A window. Codex can then record both deployment IDs/digests, pre/post probes, restoration health, operator/time, and the rollback receipt. The earlier Coolify health-check rollback is retained as a failure-recovery receipt but does not satisfy this drill.
7. **Complete human/publication gates (qualified reviewers and owner):** return the legal/privacy/licensing/trademark, factual/institutional/people/project/funding, visual-deviation, media/font-rights, publication, and feed/syndication approvals listed below. Codex can reconcile approved IDs, rerun evidence, and validate feeds after the approvals; it cannot grant them.
